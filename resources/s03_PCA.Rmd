---
title: "Compare ABC, CNN, and DNN for predicting time to fixation"
output:
  html_document:
    df_print: paged
---
...
#my_path = "/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_constant_2/"
#my_path = "/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_growth_2/"
#my_path = "/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_decay/"
#my_path = "/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_cycle/"
my_path = "/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_chaos/"

#output_path = paste("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/", today, "/", "constant/",sep = "")
#output_path = paste("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/", today, "/", "growth/",sep = "")
#output_path = paste("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/", today, "/", "decay/",sep = "")
#output_path = paste("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/", today, "/", "cycle/",sep = "")
output_path = paste("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/", today, "/", "chaos/",sep = "")
```
...
train_abc = function(train_sample, val_sample, outcomes, summary_stats, tol, method){
  ...
  for(i in 1:nrow(val_stats)){
    ...
    # extract summary statistics about prediction
...
compare_predictions_truth_scatter = function(predictions, truth, model_labels, truthlab, predlab, colorvar, colorlab){
  ...
  # for extremely low p-values, signif() has unexpected behavior
  # so just cutoff the values at that point
  if(my_cor_coeff$p.value < 1e-300){
    cor_pvalue = 0
  } else {
...
# Have tf and ta as two axes and then summary statistic as color in grid
# Will be useful to summarize how summary statistics relate to tf and ta
split_tf_ta_func_binned = function(data,tf,ta,stat_values,statname,tflab,talab,textsize){
...
# same as above but with points instead of bins
# useful if overplotting is not a problem
split_tf_ta_func_points = function(data,tf,ta,stat_values,statname,tflab,talab){
  ggplot(data,aes(x = log10(tf), y = log10(ta), color = stat_values)) +
  theme_classic() +
  geom_point() +
  scale_color_gradientn(colors = scico(10, palette = "imola"), name = statname) +
  theme(text = element_text(size = 16)) +
  labs(x = tflab, y = talab)
}

# pairwise correlations between variables
pairwise_correlation_matrix = function(data, columns, meth, column_names, legend_title, text_size, legend_size){

  mycorr = cor(data[,columns], method = meth)

  rownames(mycorr) = column_names

  colnames(mycorr) = column_names

  ggcorrplot(mycorr, hc.order = TRUE, type = "lower",
   outline.col = "white",
   ggtheme = ggplot2::theme_linedraw(),
   colors = c("#FFCE66", "#572948", "#80E6FF"), 
...
# partial R^2 per statistic
partial_r2_per_stat = function(data, text_size){
    mod_full = lm(log10(tf + ta) ~ S + log10(pi) + log10(thetaw) + tajd + tajd_var + num_haplos + h1 + h2 + h12 + h123 + h2h1 + gkl_var + gkl_skew + gkl_kurt + hscan + zns + log10(omega), data)
    
    drop_stats = drop1(mod_full)
    
    rss_full = drop_stats$RSS[1]
    
    rss_red = drop_stats$RSS[-1]
    
    partrsq = (rss_red - rss_full)/rss_red
    
    stat_names = attributes(drop_stats)$row.names[-1]
  ...
  # Ordered bar plot
  ggplot(plotdata, aes(x = reorder(stat, -partrsq), y = partrsq)) +
    geom_bar(stat = "identity", fill = "skyblue") +
    theme_classic() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
      text = element_text(size=text_size)) +
    labs(x = "Statistic", y = "Parital R squared\nfor log10(tf+ta)")
}
...
```{r}
# load ML results
train_pred_vs_act = read.table(paste(my_path,"train_predicted_vs_actual.txt", sep = ""), col.names = c("ID", "tf", "tf_mean", "tf_std"))

val_pred_vs_act = read.table(paste(my_path,"val_predicted_vs_actual.txt", sep = ""), col.names = c("ID", "tf", "tf_mean", "tf_std"))
...
train_pred_vs_act = merge(strat_sample[,c("ID", "ta")], train_pred_vs_act, by = "ID")

val_pred_vs_act = merge(strat_sample[,c("ID", "ta")], val_pred_vs_act, by = "ID")

test_pred_vs_act = merge(strat_sample[,c("ID", "ta")], test_pred_vs_act, by = "ID")
...
```{r}
# load data
train_pred_vs_act_dense = read.table(paste(my_path,"train_predicted_vs_actual_dense.txt", sep = ""))

val_pred_vs_act_dense = read.table(paste(my_path,"val_predicted_vs_actual_dense.txt", sep = ""))
...
plot_best_cnn = compare_predictions_truth_scatter(test_pred_vs_act$tf_mean, test_pred_vs_act$tf, "CNN", "log10(True tf)", "log10(Predicted tf)", log10(test_sample$ta),"log10\nsweep\nage")

plot_best_dnn = compare_predictions_truth_scatter(test_pred_vs_act_dense$tf_mean, test_pred_vs_act_dense$tf, "DNN", "log10(True tf)", "log10(Predicted tf)", log10(test_sample$ta), "log10\nsweep\nage")
...
ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/", today, "/best_models_", sim_scenario, "_", today, ".png", sep = ""), height = 5, width = 14, dpi = 300)
```
...
## main figure: tf vs ta definition
```{r}
...
## main figure: overview of all data

```{r}

strat_sample_1 = read.table("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_constant_2/stratified_sample.tsv", header = T, sep = "\t")

strat_sample_2 = read.table("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_growth_2/stratified_sample.tsv", header = T, sep = "\t")

summary_stats = c("S", "pi", "thetaw", "tajd", "tajd_var", "num_haplos", "h1", "h2", "h12", "h123", "h2h1", "gkl_var", "gkl_skew", "gkl_kurt", "hscan", "zns", "omega")

p0 = pairwise_correlation_matrix(strat_sample_1, 
                            summary_stats, 
                            "spearman", 
                            c("S", "pi", "thetaw", "tajd", "tajd_var", "num_haplos", "h1", "h2", "h12", "h123", "h2h1", "gkl_var", "gkl_skew", "gkl_kurt", "hscan", "zns", "omega"),
                            "Spearman\ncorrelation",
                            6,
                            12)

p1 = partial_r2_per_stat(strat_sample_1, 14)

#p1 = split_tf_ta_func_binned(strat_sample_1, strat_sample_1$tf, strat_sample_1$ta, strat_sample_1$hscan, "Messer's\nhscan", "log10(Fixation time)", "log10(Sweep age)", 16)

p2 = pairwise_correlation_matrix(strat_sample_2, 
                            summary_stats, 
                            "spearman", 
                            c("S", "pi", "thetaw", "tajd", "tajd_var", "num_haplos", "h1", "h2", "h12", "h123", "h2h1", "gkl_var", "gkl_skew", "gkl_kurt", "hscan", "zns", "omega"),
                            "Spearman\ncorrelation",
...
#p3 = split_tf_ta_func_binned(strat_sample_2, strat_sample_2$tf, strat_sample_2$ta, strat_sample_2$hscan, "Messer's\nhscan", "log10(Fixation time)", "log10(Sweep age)", 16)

p3 = partial_r2_per_stat(strat_sample_2, 14)

p02 =  ggarrange(p0, p2, nrow = 1, ncol = 2, common.legend = T, legend = "right", labels = c("A", "B"), font.label = list(size = 16))

p13 = ggarrange(p1, p3, nrow = 1, ncol = 2, common.legend = T, legend = "right", labels = c("C", "D"), font.label = list(size = 16))

ggarrange(p02, p13, nrow = 2, ncol = 1)

ggsave(paste("../results/", today, "/overview.png", sep = ""), dpi = 400, width = 11, height = 9)


```

## supplemental figure: pairwise correlations and partial r2 of statistics

```{r}
strat_sample_decay = read.table("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_decay/stratified_sample.tsv", header = T, sep = "\t")

strat_sample_cycle = read.table("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_cycle/stratified_sample.tsv", header = T, sep = "\t")

strat_sample_chaos = read.table("/mnt/home/robe1195/Josephs_Lab_Projects/slim-sweep-cnn/results/manuscript/outcrossing_chaos/stratified_sample.tsv", header = T, sep = "\t")

summary_stats = c("S", "pi", "thetaw", "tajd", "tajd_var", "num_haplos", "h1", "h2", "h12", "h123", "h2h1", "gkl_var", "gkl_skew", "gkl_kurt", "hscan", "zns", "omega")

## decay

p_cm_decay = pairwise_correlation_matrix(strat_sample_decay, 
                            summary_stats, 
                            "spearman", 
                            c("S", "pi", "thetaw", "tajd", "tajd_var", "num_haplos", "h1", "h2", "h12", "h123", "h2h1", "gkl_var", "gkl_skew", "gkl_kurt", "hscan", "zns", "omega"),
                            "Spearman\ncorrelation",
...
p_r2_decay = partial_r2_per_stat(strat_sample_decay, 14)

## cycle

p_cm_cycle = pairwise_correlation_matrix(strat_sample_cycle, 
                            summary_stats, 
                            "spearman", 
                            c("S", "pi", "thetaw", "tajd", "tajd_var", "num_haplos", "h1", "h2", "h12", "h123", "h2h1", "gkl_var", "gkl_skew", "gkl_kurt", "hscan", "zns", "omega"),
                            "Spearman\ncorrelation",
...
p_r2_cycle = partial_r2_per_stat(strat_sample_cycle, 14)

## chaos

p_cm_chaos = pairwise_correlation_matrix(strat_sample_chaos, 
                            summary_stats, 
                            "spearman", 
                            c("S", "pi", "thetaw", "tajd", "tajd_var", "num_haplos", "h1", "h2", "h12", "h123", "h2h1", "gkl_var", "gkl_skew", "gkl_kurt", "hscan", "zns", "omega"),
                            "Spearman\ncorrelation",
...
p_r2_chaos = partial_r2_per_stat(strat_sample_chaos, 14)

## final image

p_cm = ggarrange(p_cm_decay, p_cm_cycle, p_cm_chaos, common.legend = T, labels = c("A", "B", "C"), nrow = 1, ncol = 3, font.label = list(size = 16), legend = "right")

p_r2 = ggarrange(p_r2_decay, p_r2_cycle, p_r2_chaos, nrow = 1, ncol = 3, labels = c("D", "E", "F"), font.label = list(size = 16))

ggarrange(p_cm, p_r2, nrow = 2, ncol = 1)

ggsave(paste("../results/", today, "/correlations_partrsq_decay_cycle_chaos.png", sep = ""), dpi = 400, width = 13, height = 7)
```
...
test_pred_vs_act$sqr_error = (test_pred_vs_act$tf - test_pred_vs_act$tf_mean)^2

### plot prediction error vs true values
pred_error_cnn = split_tf_ta_func_points(test_pred_vs_act, test_pred_vs_act$tf, test_pred_vs_act$ta, test_pred_vs_act$sqr_error, "Squared tf\nprediction\nerror", "log10(True tf)", "log10(True ta)")
...
### calculate prediction error
test_pred_vs_act_dense$sqr_error = (test_pred_vs_act_dense$tf - test_pred_vs_act_dense$tf_mean)^2

### plot prediction error vs true values
pred_error_dnn = split_tf_ta_func_points(test_pred_vs_act_dense, test_pred_vs_act_dense$tf, test_pred_vs_act_dense$ta, test_pred_vs_act_dense$sqr_error, "Squared tf\nprediction\nerror", "log10(True tf)", "log10(True ta)")

## ABC

### add ta info
best_abc = merge(strat_sample[,c("ID", "ta")], best_abc, by = "ID")

### calculate prediction error
best_abc$sqr_error = (log10(best_abc$truth) - log10(best_abc$pred_median))^2

### plot tf, vs ta, vs prediction error
pred_error_abc = split_tf_ta_func_points(best_abc, log10(best_abc$truth), log10(best_abc$ta), best_abc$sqr_error, "Squared tf\nprediction\nerror", "log10(True tf)", "log10(True ta)")
 
## plot all together
ggarrange(pred_error_cnn, pred_error_dnn, pred_error_abc, nrow = 1, ncol = 3, common.legend = T)
