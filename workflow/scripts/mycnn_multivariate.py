...
def build_model(hp):
  ...
  conv1 = keras.layers.Conv2D(filters = hp.Int("conv1-filters", min_value=16, max_value=128, step=8), kernel_size = 7, strides = 2, padding = "same", activation = "relu", input_shape = [128,128,3])(input_A)
  ...
  conv2 = keras.layers.Conv2D(filters = hp.Int("conv2-filters", min_value=16, max_value=128, step=8), kernel_size = 3, strides = 1, padding = "same", activation = "relu")(pool1)
  ...
  conv3 = keras.layers.Conv2D(filters = hp.Int("conv3-filters", min_value=16, max_value=128, step=8), kernel_size = 3, strides = 1, padding = "same", activation = "relu")(pool2)
  ...
  dense_A = keras.layers.Dense(units=hp.Int("denseA-units", min_value=32, max_value=512, step=16), activation = "relu")(flat)
  dense_A = keras.layers.Dropout(hp.Float(name = "denseA-dropout", min_value=0, max_value=0.99))(dense_A)
  dense_B = keras.layers.Dense(units=hp.Int("denseB-units", min_value=32, max_value=512, step=16), activation = "relu")(input_B)
  ...
  full = keras.layers.Dense(units=hp.Int("full-units", min_value=32, max_value=512, step=16), activation = "relu")(concat)
  full = keras.layers.Dropout(hp.Float(name = "full-dropout", min_value=0, max_value=0.99))(full)
  output = keras.layers.Dense(2, name = "output")(full)
...
tuner.search((train_images, train_pos), (train_params["tf"], train_params["ta"]), epochs=2, validation_data=((val_images, val_pos), (val_params["tf"], val_params["ta"])))
...
train_generator = createGenerator(train_params, train_pos, batch_size, "data/images/", "ID", ["tf", "ta"])
val_generator = createGenerator(val_params, val_pos, batch_size, "data/images/", "ID", ["tf", "ta"])
...
np.savetxt('test_predicted_vs_actual.txt', np.c_[test_ids, test_params["tf"], test_params["ta"], test_pred_mean, test_pred_std], header = "ID true_tf true_ta pred_tf_mean pred_ta_mean pred_tf_std pred_ta_std")
...
np.savetxt('val_predicted_vs_actual.txt', np.c_[val_ids, val_params["tf"], val_params["ta"], val_pred_mean, val_pred_std], header = "ID true_tf true_ta pred_tf_mean pred_ta_mean pred_tf_std pred_ta_std")
...
np.savetxt('train_predicted_vs_actual.txt', np.c_[train_ids, train_params["tf"], train_params["ta"], train_pred_mean, train_pred_std], header = "ID true_tf true_ta pred_tf_mean pred_ta_mean pred_tf_std pred_ta_std")
