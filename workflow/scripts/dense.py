...
def build_model(hp):
  ...
  input = keras.layers.Input(shape = [28], name = "summaries")
  dense_A = keras.layers.Dense(units=hp.Int("denseA-units", min_value=32, max_value=512, step=8), activation = "relu")(input)
  dense_A = keras.layers.Dropout(hp.Float(name = "denseA-dropout", min_value=0, max_value=0.99))(dense_A)
  dense_B = keras.layers.Dense(units=hp.Int("denseB-units", min_value=32, max_value=512, step=8), activation = "relu")(dense_A)
  dense_B = keras.layers.Dropout(hp.Float(name = "denseB-dropout", min_value=0, max_value=0.99))(dense_B)
  dense_C = keras.layers.Dense(units=hp.Int("denseC-units", min_value=32, max_value=512, step=8), activation = "relu")(dense_B)
