...
train_abc = function(train_sample, val_sample, outcomes, summary_stats, tol, method){
  ...
  for(i in 1:nrow(val_stats)){
    ...
    cred_int = round(c(summary(result)[[2]], summary(result)[[6]]))
    median_est = round(summary(result)[[3]])
    mean_est = round(summary(result)[[4]])
    mode_est = round(summary(result)[[5]])
...
ggplot(params, aes(x = log10(tf/ta))) +
  geom_density() +
  theme_classic()

ggplot(params, aes(x = log10((tf/(tf + ta))))) +
  geom_density() +
  theme_classic()
...
ggplot(params, aes(x = log10((tf/(tf + ta))), y = log10(pi))) +
  geom_point() +
  theme_classic() +
  theme(text = element_text(size = 20))
...
for(trained_mod in trained_mods){
  compare_predictions_truth_scatter(log10(trained_mod$pred_mean), log10(trained_mod$truth), paste(name(trained_mod), ", Mean", sep = ""), "log10(True tf)", "log10(Predicted tf)")
  
  compare_predictions_truth_scatter(log10(trained_mod$pred_median), log10(trained_mod$truth), paste(name(trained_mod), ", Median", sep = ""), "log10(True tf)", "log10(Predicted tf)")

compare_predictions_truth_scatter(log10(trained_mod$pred_mode), log10(trained_mod$truth), paste(name(trained_mod), ", Mode", sep = ""), "log10(True tf)", "log10(Predicted tf)")
}
```
...
compare_predictions_truth(test_pred_vs_act$V3, test_pred_vs_act$V2, "CNN, test set", "log10(true tf)", "log10(predicted tf)")
...
ggplot(aes(y = V3, x = V2, color = log10(ta)), data = test_pred_vs_act) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "#8C0172", linewidth = 2) +
...
# scatterplot
compare_predictions_truth(test_pred_vs_act_dense$V3, test_pred_vs_act_dense$V2, "Neural Net, test set", "log10(true tf)", "log10(predicted tf)")
...
#breaks = seq(from = min(log10(params$tf/(params$tf + params$ta))) - width, to = max(log10(params$tf/(params$tf + params$ta))) + width, by = width)
...
# 
# ggplot(params[(log10(params$tf) > 2.55) & (log10(params$tf) <= 4.15),], aes(x=bin_tf, y=bin_ta) ) + 
#   geom_bin2d() +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
# 
# levels_to_keep = c("(2.55,2.65]", "(2.65,2.75]", "(2.75,2.85]", "(2.85,2.95]", "(2.95,3.05]", "(3.05,3.15]", "(3.15,3.25]", "(3.25,3.35]", "(3.35,3.45]", "(3.45,3.55]", "(3.55,3.65]", "(3.65,3.75]", "(3.75,3.85]", "(3.85,3.95]", "(3.95,4.05]", "(4.05,4.15]")
# 
...
write.table(strat_sample, "stratified_sample_20.tsv", sep = "\t", quote = F, row.names = F)
```
