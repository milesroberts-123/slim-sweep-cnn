...
ggplot(test_pred_vs_act, aes(x = test_pred_vs_act$V2, y = test_pred_vs_act$V4)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  theme_classic()

ggplot(test_pred_vs_act, aes(x = test_pred_vs_act$V3, y = test_pred_vs_act$V5)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  theme_classic()
```
...
ggplot(params[(log10(params$tf) > 1.95) & (log10(params$tf) < 4.05),], aes(x=bin_tf, y=bin_ta) ) + 
   geom_bin2d() +
   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

# Levels 
levels_to_keep = c("(2.15,2.25]", "(2.25,2.35]", "(2.35,2.45]", "(2.45,2.55]", "(2.55,2.65]", "(2.65,2.75]", "(2.75,2.85]", "(2.85,2.95]", "(2.95,3.05]", "(3.05,3.15]", "(3.15,3.25]", "(3.25,3.35]", "(3.35,3.45]", "(3.45,3.55]", "(3.55,3.65]", "(3.65,3.75]")
...
write.table(strat_sample, "stratified_sample_19.tsv", sep = "\t", quote = F, row.names = F)
```
