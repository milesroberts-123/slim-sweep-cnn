...
train_n = round(nrow(strat_sample)*0.7)
test_n = round(nrow(strat_sample)*0.1)
val_n = round(nrow(strat_sample)*0.2)
...
write.table(strat_sample, "stratified_sample_11.tsv", sep = "\t", quote = F, row.names = F)
```
...
```{r}
real_pred = read.table("../workflow/real_predictions.txt", header = T)
...
ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/real_density_", today, ".png", sep = ""), height = 7, width = 7)
