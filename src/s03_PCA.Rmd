...
error_mod = lm(log10(error) ~ N/Q + log10(sweepS*Q) + sigma + h + log10(mu*Q) + log10(R*Q) + tau/Q + B + U, data = errors)
summary(error_mod)
plot(error_mod)
...
train_n = round(nrow(strat_sample)*0.75)
test_n = round(nrow(strat_sample)*0.15)
val_n = round(nrow(strat_sample)*0.15)
...
write.table(strat_sample, "stratified_sample_10.tsv", sep = "\t", quote = F, row.names = F)
```
