...
# only keep simulations that images
...
params$scaled_expected_tfs = mapply(expected_tf, N = 19840, s = params$sweepS, h = params$h, sigma = params$sigma, Q = params$Q)

summary(params$scaled_expected_tfs)

params$unscaled_expected_tfs = mapply(expected_tf, N = 19840, s = params$sweepS, h = params$h, sigma = params$sigma, Q = 1)
...
cv.res.rej.mean <- cv4abc(data.frame(tf=params$tf), params[,c(28:31, 34:45)], nval=100, tols=c(0.01, 0.05, 0.1), method="rejection", statistic = "mean")
...
write.table(strat_sample, "stratified_sample_15.tsv", sep = "\t", quote = F, row.names = F)
```
