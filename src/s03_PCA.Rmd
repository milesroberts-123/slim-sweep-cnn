...
compare_predictions_truth_scatter = function(predictions, truth, model_labels, truthlab, predlab, colorvar, colorlab){
  ...
  scale_color_continuous(low = "#B200B2", high = "#FFFF66") +
  theme_classic() +
...
ggplot(params[(log10(params$tf) < 4),], aes(x = log10(tf), y = log10(ta))) +
  #geom_point() +
  theme_classic() +
...
mod_full = lm(log10(tf) ~ S + log10(pi) + log10(thetaw) + tajd + log10(tajd_var) + log10(num_haplos) + log10(h1) + log10(h2) + log10(h12) + log10(h123) + log10(h2h1) + log10(gkl_var) + log10(gkl_skew) + gkl_kurt + log10(hscan) + log10(zns) + log10(omega), data = strat_sample)

summary(mod_full)

plot(mod_full)

mod_reduced = lm(log10(tf) ~ log10(pi) + log10(thetaw) + tajd, data = strat_sample)
...
for(i in 1:length(trained_mods)){
  ...
  ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/", file_mod_name, "_mean_", today, ".png", sep = ""), height = 7, width = 8.5)
  ...
  ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/", file_mod_name, "_median_", today, ".png", sep = ""), height = 7, width = 8.5)
  ...
  compare_predictions_truth_scatter(log10(trained_mod$pred_mode), log10(trained_mod$truth), paste(mod_name, ", Mode", sep = ""), "log10(True tf)", "log10(Predicted tf)", log10(val_sample$ta), "log10(Sweep Age)")
    ...
    ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/", file_mod_name, "_mode_", today, ".png", sep = ""), height = 7, width = 8.5)
}
