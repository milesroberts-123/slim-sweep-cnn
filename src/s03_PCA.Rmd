...
ggplot(params, aes(x = tajd, y = tajd_incomplete)) +
  ...
  labs(x = "Tajima's D without missing data", y = "Tajima's D with missing data",color = "Type")
...
expected_tf = function(N, s, h, sigma, Q){
  ...
  return(result)
  #return(list(c, a*(b + d) + f*g))
  #return(c(result, a, b, c, d, f, g))
  #return(c)
}
...
params$scaled_expected_tfs = mapply(expected_tf, N = 78908, s = params$sweepS, h = params$h, sigma = params$sigma, Q = params$Q)

summary(params$scaled_expected_tfs)

params$unscaled_expected_tfs = mapply(expected_tf, N = 78908, s = params$sweepS, h = params$h, sigma = params$sigma, Q = 1)
...
cv.res.rej.median <- cv4abc(data.frame(tf=params$tf), params[,c(28:31, 33:44)], nval=100, tols=c(0.01, 0.05, 0.1), method="rejection", statistic = "median")

cv.res.rej.mean <- cv4abc(data.frame(tf=params$tf), params[,c(28:31, 33:44)], nval=100, tols=c(0.01, 0.05, 0.1), method="rejection", statistic = "mean")

cv.res.rej.mode <- cv4abc(data.frame(tf=params$tf), params[,c(28:31, 33:44)], nval=100, tols=c(0.01, 0.05, 0.1), method="rejection", statistic = "mode")

cv.res.reg.median <- cv4abc(data.frame(tf=params$tf), params[,28:44], nval=100, tols=c(0.01, 0.05, 0.1), method="ridge", statistic = "median")

cv.res.reg.mean <- cv4abc(data.frame(tf=params$tf), params[,28:44], nval=100, tols=c(0.01, 0.05, 0.1), method="ridge", statistic = "mean")

cv.res.reg.mode <- cv4abc(data.frame(tf=params$tf), params[,28:44], nval=100, tols=c(0.01, 0.05, 0.1), method="ridge", statistic = "mode")
...
par(mfrow=c(1,3), mar=c(5,3,4,.5), cex=.8)
plot(cv.res.rej.median, caption="Rejection - median")
plot(cv.res.rej.mean, caption="Rejection - mean")
plot(cv.res.rej.mode, caption="Rejection - mode")

par(mfrow=c(1,3), mar=c(5,3,4,.5), cex=.8)
plot(cv.res.reg.median, caption="Regression - median")
plot(cv.res.reg.mean, caption="Regression - mean")
plot(cv.res.reg.mode, caption="Regression - mode")

plot(cv.res.reg, caption="Ridge regression")
plot(cv.res.net, caption="Neural net")

cor.test(cv.res.rej$estim$tol0.01, cv.res.rej$true$tf, method = "spearman")
cor.test(cv.res.reg$estim$tol0.05, cv.res.reg$true$tf, method = "spearman")
cor.test(cv.res.net$estim$tol0.1, cv.res.net$true$tf, method = "spearman")
...
plot_abc_results = function(predictions, truth, model_labels){
  ...
  ggplot(aes(x = predictions, y = truth), data = plotdata) +
  geom_point() +
...
rej_fig1 = plot_abc_results(cv.res.rej.median$estim$tol0.01, cv.res.rej.median$true$tf, "Rejection, Median, Tol = 0.01")

rej_fig2 = plot_abc_results(cv.res.rej.median$estim$tol0.05, cv.res.rej.median$true$tf, "Rejection, Median, Tol = 0.05")

rej_fig3 = plot_abc_results(cv.res.rej.median$estim$tol0.1, cv.res.rej.median$true$tf, "Rejection, Median, Tol = 0.1")

ggarrange(rej_fig1, rej_fig2, rej_fig3, nrow = 2, ncol = 2)
ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/abc_rejection_", today, ".png", sep = ""), height = 8, width = 7)
...
write.table(strat_sample, "stratified_sample_14.tsv", sep = "\t", quote = F, row.names = F)
```
...
```{r}
fst_files = list.files(path = "/mnt/scratch/robe1195/Josephs_Lab_Projects/selection-demography-cnn/workflow/", pattern = "_fst.txt", full.names = T)
...
# calculate PBS for north sweden, relative to south sweden and relicts
fst_north_south = fst_table[((fst_table$pop1 %in% c("north_sweden", "south_sweden")) & (fst_table$pop2 %in% c("north_sweden", "south_sweden"))),]
fst_north_relict = fst_table[((fst_table$pop1 %in% c("north_sweden", "relict")) & (fst_table$pop2 %in% c("north_sweden", "relict"))),]
fst_south_relict = fst_table[((fst_table$pop1 %in% c("relict", "south_sweden")) & (fst_table$pop2 %in% c("relict", "south_sweden"))),]

pbs_table = merge(fst_north_south, fst_north_relict, by = c("chromosome", "window_pos_1", "window_pos_2"))
pbs_table = merge(pbs_table, fst_south_relict, by = c("chromosome", "window_pos_1", "window_pos_2"))
...
ggplot(pbs_table[(pbs_table$chromosome == 2),], aes(x = (window_pos_1 + window_pos_2)/2, y = pbs)) +
  ...
  geom_hline(yintercept = top_cutoff, lwd = 2) +
  theme_classic() +
  labs(x = "Middle of window", y = "Population branch statistic\nNorth Sweden focus", title = "Chromosome 2")
...
ggplot(pbs_table[(pbs_table$chromosome == 3),], aes(x = (window_pos_1 + window_pos_2)/2, y = pbs)) +
  ...
  geom_hline(yintercept = top_cutoff, lwd = 2) +
  theme_classic() +
  labs(x = "Middle of window", y = "Population branch statistic\nNorth Sweden focus", title = "Chromosome 3")
...
ggplot(pbs_table[(pbs_table$chromosome == 4),], aes(x = (window_pos_1 + window_pos_2)/2, y = pbs)) +
  ...
  geom_vline(xintercept = 6637000, color = "red") +
  theme_classic() +
  labs(x = "Middle of window", y = "Population branch statistic\nNorth Sweden focus", title = "Chromosome 4")
...
ggplot(pbs_table[(pbs_table$chromosome == 5),], aes(x = (window_pos_1 + window_pos_2)/2, y = pbs)) +
  ...
  geom_vline(xintercept = 26166000, color = "red") +
  theme_classic() +
  labs(x = "Middle of window", y = "Population branch statistic\nNorth Sweden focus", title = "Chromosome 5")
```
