...
compare_predictions_truth_scatter = function(predictions, truth, model_labels, truthlab, predlab){
  ...
  p = ggplot(aes(y = predictions, x = truth), data = plotdata) +
  geom_point() +
...
```{r}
width = 0.1
max_bin_height = 400 # adjust so that observations at log10(20000) ~ 4.31 are included
...
# linear models
```{r}
# if strat_sample not already available, load from previous session
#strat_sample = read.table(paste(my_path, "workflow/stratified_sample.tsv", sep = ""), sep = "\t", header = T)

#strat_sample = strat_sample[is.finite(strat_sample$pi),]

mod = lm(log10(tf) ~ S + log10(pi) + log10(thetaw) + tajd + tajd_var + num_haplos + h1 + h2 + h12 + h123 + h2h1 + gkl_var + gkl_skew + gkl_kurt + hscan + zns + omega, data = strat_sample)

mod = lm(log10(tf) ~ log10(pi), data = strat_sample)

mod = lm(log10(tf) ~ hscan, data = strat_sample)
...
back_mod = lm(formula = log10(tf) ~ S + log10(pi) + log10(thetaw) + tajd + 
    tajd_var + num_haplos + h1 + h2 + h12 + h123 + h2h1 + gkl_var + 
    gkl_skew + hscan + zns + omega, data = strat_sample)
...
for(i in 1:length(trained_mods)){
  ...
  print("Plotting mean...")
  compare_predictions_truth_scatter(log10(trained_mod$pred_mean), log10(trained_mod$truth), paste(mod_name, ", Mean", sep = ""), "log10(True tf)", "log10(Predicted tf)")
  
  ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/", file_mod_name, "_mean_", today, ".png", sep = ""), height = 7, width = 7)
 
  print("Plotting median...") 
  compare_predictions_truth_scatter(log10(trained_mod$pred_median), log10(trained_mod$truth), paste(mod_name, ", Median", sep = ""), "log10(True tf)", "log10(Predicted tf)")
  
  ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/", file_mod_name, "_median_", today, ".png", sep = ""), height = 7, width = 7)

  print("Plotting mode...")
  compare_predictions_truth_scatter(log10(trained_mod$pred_mode), log10(trained_mod$truth), paste(mod_name, ", Mode", sep = ""), "log10(True tf)", "log10(Predicted tf)")

    ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/", file_mod_name, "_mode_", today, ".png", sep = ""), height = 7, width = 7)
}
...
best_abc = train_abc(train_sample, test_sample, "tf", summary_stats, 0.01, "rejection")

compare_predictions_truth_scatter(log10(best_abc$pred_median), log10(best_abc$truth), "Rej., Median, tol = 0.01", "log10(True tf)", "log10(Predicted tf)")

ggsave(paste("/mnt/home/robe1195/Josephs_Lab_Projects/selection-demography-cnn/results/", today, "/abc_rejection_median_tol001_", today, ".png", sep = ""), height = 7, width = 7)
```
...
train_pred_vs_act = read.table(paste(my_path,"workflow/train_predicted_vs_actual.txt", sep = ""))

val_pred_vs_act = read.table(paste(my_path,"workflow/val_predicted_vs_actual.txt", sep = ""))

test_pred_vs_act = read.table(paste(my_path,"workflow/test_predicted_vs_actual.txt", sep = ""))

# merge in sweep ages to see how predictions falter with older sweeps
#names(params)[1] = "V1"
#test_pred_vs_act = merge(params[,c("V1", "ta")], test_pred_vs_act, by = "V1")
...
##training set
compare_predictions_truth_scatter(train_pred_vs_act$V3, train_pred_vs_act$V2, "CNN, training set", "log10(true tf)", "log10(predicted tf)")
...
## validation set
compare_predictions_truth_scatter(val_pred_vs_act$V3, val_pred_vs_act$V2, "CNN, validation set", "log10(true tf)", "log10(predicted tf)")
...
## test set
compare_predictions_truth_scatter(test_pred_vs_act$V3, test_pred_vs_act$V2, "CNN, test set", "log10(true tf)", "log10(predicted tf)")
...
```{r}
# load data
train_pred_vs_act_dense = read.table("/mnt/scratch/robe1195/Josephs_Lab_Projects/selection-demography-cnn/workflow/train_predicted_vs_actual_dense.txt")

val_pred_vs_act_dense = read.table("/mnt/scratch/robe1195/Josephs_Lab_Projects/selection-demography-cnn/workflow/val_predicted_vs_actual_dense.txt")

test_pred_vs_act_dense = read.table("/mnt/scratch/robe1195/Josephs_Lab_Projects/selection-demography-cnn/workflow/test_predicted_vs_actual_dense.txt")
...
# scatterplot
compare_predictions_truth_scatter(test_pred_vs_act_dense$V3, test_pred_vs_act_dense$V2, "Neural Net, test set", "log10(true tf)", "log10(predicted tf)")
